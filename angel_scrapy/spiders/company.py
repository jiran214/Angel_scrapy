...
class CompanySpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        for c in company:
            yield scrapy.Request(url=base_url % c, headers=self.headers, cookies=cookie,callback=self.search_parse,
                                 meta={'keyword': c})
    ...
    def search_parse(self, response):
        ...
        if hrefs:
            ...
            if len(hrefs) == 1:
                print('%s匹配到一个company' % keyword)
            else:
                print('%s关键词匹配多个company' % keyword)

        else:
            print('%s关键词没有匹配到company' % keyword)
